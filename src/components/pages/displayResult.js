...
const DisplayData = (props) => {
  ...
  let getCarData;
  if (services) {
    getFlightData = services.filter(
      (service) => service.provider.serviceType === "flight"
    );
    getRailData = services.filter(
      (service) => service.provider.serviceType === "rail"
    );
    getBusData = services.filter(
      (service) => service.provider.serviceType === "bus"
    );
    getCarData = services.filter(
      (service) => service.provider.serviceType === "car"
    );
  }
  const ShowDetail = () => {
    let service;
      ...
      <div>
        <div className="card-header">
          <h5 className="card-title">
            Provider: {service ? service.provider.serviceProvider : ""}
          </h5>
        </div>
        <div className="card-body">
          ...
          <p>
            Duration:{" "}
            {getTimeDuration(service ? service.estimateTimeOfArrival : 0)}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="get-info">
      <div className="header">
        <h2>
          Information About Travels from {origin} to {destination}{" "}
        </h2>
      </div>
      <Modal show={show} handleClose={hideModal}>
        <div className="card modal-card">
          ...
          <button onClick={handleInvoice} className="btn btn-outline-success">
            Make Payment
          </button>
        </div>
      </Modal>
      {services ? (
            ...
            <table className="table table-bordered table-hover">
              ...
              <tbody>
                {getFlightData ? (
                  ...
                  <tr>
                    <td> Sorry No Service Provider For that Location</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <h4 className="text-danger text-center">
          Sorry, We do not have Service Provider yet for the location
        </h4>
      )}
  ...
  return (
    <div>
      ...
      {searchData ? (
        ...
        <h3>Sorry No Information Yet </h3>
      )}
    </div>
  );
};
